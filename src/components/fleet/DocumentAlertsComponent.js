                ...
                >
                  <div className="flex justify-between items-start">
                    <div className="flex-1 min-w-0">
                      <p className="font-medium text-gray-900 dark:text-gray-100 text-sm truncate">{item.driver}</p>
                      <p className="text-sm text-gray-500 dark:text-gray-400">{item.type} expires on {formatDate(item.expiryDate)}</p>
                    </div>
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ml-2 ${item.daysRemaining <= 7
