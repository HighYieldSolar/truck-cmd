    ...
    <div className="mb-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between bg-gradient-to-r from-blue-700 to-indigo-800 p-8 rounded-xl shadow-lg text-white">
        <div>
          ...
          <p className="text-blue-100 mt-2 text-lg">{t('header.subtitle')}</p>
          <div className="flex items-center text-sm text-blue-200 mt-3">
            <Link href="/dashboard" className="hover:text-white transition-colors">{t('header.breadcrumb.dashboard')}</Link>
            <ChevronRight size={16} className="mx-2" />
            <span className="text-white font-medium">{t('header.breadcrumb.invoices')}</span>
          </div>
        </div>
      ...
      <div className="mt-6 bg-white p-5 rounded-xl shadow-md border border-gray-200">
        <p className="text-gray-700 flex items-center">
          ...
          <span className="font-medium">{t('header.description')}</span>
        </p>
      </div>
    </div>
  );
