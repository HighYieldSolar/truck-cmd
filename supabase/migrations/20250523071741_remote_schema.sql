...
CREATE OR REPLACE FUNCTION public.create_notification(p_user_id uuid, p_title text, p_message text, p_notification_type text, p_entity_type text DEFAULT NULL::text, p_entity_id text DEFAULT NULL::text, p_link_to text DEFAULT NULL::text, p_due_date timestamp with time zone DEFAULT NULL::timestamp with time zone, p_urgency text DEFAULT 'NORMAL'::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
CREATE OR REPLACE FUNCTION public.get_all_notifications(p_user_id uuid, p_page_number integer DEFAULT 1, p_page_size integer DEFAULT 15, p_filter_types text[] DEFAULT NULL::text[], p_filter_read_status boolean DEFAULT NULL::boolean)
 RETURNS TABLE(id uuid, user_id uuid, title text, message text, is_read boolean, read_at timestamp with time zone, notification_type text, entity_type text, entity_id text, created_at timestamp with time zone, link_to text, due_date timestamp with time zone, urgency text, total_count bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
DECLARE
...
BEGIN
  ...
  -- Return the notifications with pagination and filtering
  RETURN QUERY
  SELECT 
    n.*,
  ...
  WHERE n.user_id = p_user_id
    AND (p_filter_types IS NULL OR n.notification_type = ANY(p_filter_types))
    AND (p_filter_read_status IS NULL OR n.is_read = p_filter_read_status)
...
BEGIN
    ...
    -- Get top 5 recent unread notifications
    SELECT jsonb_agg(t)
    INTO recent_notifications
    FROM (
        SELECT id, title, message, created_at, notification_type, link_to, urgency, entity_id, entity_type, due_date, is_read
        FROM public.notifications
        WHERE user_id = p_user_id AND is_read = FALSE
        ORDER BY created_at DESC
        LIMIT 5
    ...
    RETURN jsonb_build_object(
        ...
        'recent_notifications', COALESCE(recent_notifications, '[]'::jsonb)
    );
END;
$function$
;
